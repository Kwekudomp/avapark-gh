...
export default function VideosCMSClient({ initialVideos }: { initialVideos: CMSVideo[] }) {
  ...
  const [confirmDelete, setConfirmDelete] = useState<CMSVideo | null>(null);
  ...
  async function handleVideoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (data.url) {
        setUpForm(f => ({ ...f, video_url: data.url }));
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.video) {
        ...
        toast("error", `Could not save video: ${data.error ?? res.status}`);
      }
    } catch {
      toast("error", "Could not save video — check your connection and try again.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleToggle(v: CMSVideo) {
    try {
      ...
      if (res.ok) {
        ...
        toast("success", v.is_active ? "Video hidden from the site." : "Video is now live.");
      } else {
        toast("error", `Could not update video (${res.status}).`);
      }
    } catch {
      toast("error", "Could not update video — check your connection and try again.");
    }
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast("error", `Could not remove video (${res.status}).`);
      }
    } catch {
      toast("error", "Could not remove video — check your connection and try again.");
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-dark">Videos</h1>
          <p className="text-sm text-text-secondary mt-0.5">{videos.length} video{videos.length !== 1 ? "s" : ""}</p>
        </div>
        <button
        ...
        >
          {adding ? <><X className="w-4 h-4" aria-hidden /> Cancel</> : <><Plus className="w-4 h-4" aria-hidden /> Add Video</>}
        </button>
      </div>
        ...
        <form onSubmit={handleSave} className="bg-white rounded-2xl border border-border p-6 space-y-4">
          ...
          <div className="flex gap-2 p-1 bg-bg-alt rounded-xl w-fit">
            <button type="button" onClick={() => setSource("youtube")}
              ...
              className={`min-h-11 px-4 rounded-lg text-sm font-medium transition-colors cursor-pointer ${source === "youtube" ? "bg-white shadow text-primary" : "text-text-secondary hover:text-dark"}`}>
              YouTube Link
            </button>
            <button type="button" onClick={() => setSource("upload")}
              aria-pressed={source === "upload"}
              className={`min-h-11 px-4 rounded-lg text-sm font-medium transition-colors cursor-pointer ${source === "upload" ? "bg-white shadow text-primary" : "text-text-secondary hover:text-dark"}`}>
              Upload Video
            </button>
          </div>
          ...
          {source === "youtube" ? (
            <>
              <input required value={ytForm.title} onChange={e => setYtForm(f => ({ ...f, title: e.target.value }))} placeholder="Video title" aria-label="Video title" className={inputClass} />
              <input required value={ytForm.youtube_url} onChange={e => setYtForm(f => ({ ...f, youtube_url: e.target.value }))}
                placeholder="YouTube URL (e.g. https://youtu.be/abc123)" aria-label="YouTube URL" className={inputClass} />
              <select value={ytForm.category} onChange={e => setYtForm(f => ({ ...f, category: e.target.value }))} aria-label="Video category" className={inputClass}>
            ...
            <>
              <input required value={upForm.title} onChange={e => setUpForm(f => ({ ...f, title: e.target.value }))} placeholder="Video title" aria-label="Video title" className={inputClass} />
              <select value={upForm.category} onChange={e => setUpForm(f => ({ ...f, category: e.target.value }))} aria-label="Video category" className={inputClass}>
                {CATEGORIES.map(c => <option key={c} value={c} className="capitalize">{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}
              </select>
              <div>
                {upForm.video_url ? (
                  <div className="space-y-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                      className="min-h-11 px-2 text-sm text-accent hover:underline cursor-pointer">Replace video</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                    className="w-full border-2 border-dashed border-border rounded-xl py-10 text-text-secondary hover:border-primary hover:text-primary transition-colors cursor-pointer disabled:opacity-60 text-sm">
                    {uploading ? "Uploading…" : "Click to upload video (MP4, MOV, WEBM — max 50MB)"}
                  </button>
                )}
                <input ref={fileInputRef} type="file" accept="video/mp4,video/mov,video/webm,video/quicktime" aria-label="Choose video file" onChange={handleVideoUpload} className="hidden" />
              </div>
            </>
          )}
      ...
      {videos.length === 0 && !adding ? (
        <div className="bg-white rounded-2xl border-2 border-dashed border-border text-center py-16 text-text-secondary">
          <Clapperboard className="w-10 h-10 mx-auto mb-3 opacity-40" strokeWidth={1.5} aria-hidden />
                ...
                <div className="p-4">
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => handleToggle(v)}
                      ...
                      className={`min-h-11 px-3 rounded-full text-xs font-medium transition-colors cursor-pointer ${v.is_active ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-bg-alt text-text-secondary hover:bg-border"}`}>
      ...
      <ConfirmDialog
        ...
        message={confirmDelete ? `"${confirmDelete.title}" will be removed from the site.` : ""}
