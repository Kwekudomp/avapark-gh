...
export default function InquiriesCMSClient({ initialInquiries }: { initialInquiries: Inquiry[] }) {
  ...
  async function patch(id: string, updates: Partial<Pick<Inquiry, "status" | "admin_note">>, opts?: { quiet?: boolean }) {
    setBusyId(id);
    try {
      ...
      if (data.inquiry) {
        ...
        toast("error", `Could not update inquiry: ${data.error ?? res.status}`);
      }
    } catch {
      if (!opts?.quiet) toast("error", "Could not update inquiry — check your connection and try again.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function markRead(inq: Inquiry) {
    if (inq.status === "unread") await patch(inq.id, { status: "read" }, { quiet: true });
  }

  async function setStatus(inq: Inquiry, status: InquiryStatus) {
    await patch(inq.id, { status });
    toast("success", status === "archived" ? "Inquiry archived." : "Inquiry moved back to inbox.");
  }
  ...
  async function remove(inq: Inquiry) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast("error", `Could not delete inquiry (${res.status}).`);
      }
    } catch {
      toast("error", "Could not delete inquiry — check your connection and try again.");
    } finally {
      setBusyId(null);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-end justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-dark">Inquiries</h1>
          <p className="text-sm text-text-secondary mt-0.5">{inquiries.length} inquir{inquiries.length === 1 ? "y" : "ies"} from the contact form</p>
        </div>
        {unreadCount > 0 && (
          <span className="bg-accent text-white text-xs font-semibold px-3 py-1 rounded-full shrink-0">
            {unreadCount} unread
          </span>
        )}
      </div>
      ...
      <div className="flex gap-2 mb-5">
        {[
          ...
          <button
            ...
            className={`min-h-11 px-4 rounded-full text-sm font-medium transition-colors cursor-pointer ${
              tab === t.key
        ...
        <div className="bg-white rounded-2xl border border-border overflow-hidden">
          {filtered.length === 0 ? (
            <div className="p-10 text-center text-text-secondary">
              <Inbox className="w-10 h-10 mx-auto mb-3 opacity-40" strokeWidth={1.5} aria-hidden />
              ...
              {filtered.map(inq => (
                <li key={inq.id}>
                  <button
                    ...
                    className={`w-full text-left p-4 min-h-11 transition-colors cursor-pointer ${
        ...
        <div className="bg-white rounded-2xl border border-border p-6">
          {!selected ? (
            <div className="h-full flex flex-col items-center justify-center text-center text-text-secondary py-20">
              <Mail className="w-10 h-10 mb-3 opacity-40" strokeWidth={1.5} aria-hidden />
            ...
            <>
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="flex gap-2 flex-wrap">
                  <a
                    href={replyHref}
                    className="inline-flex items-center gap-1.5 min-h-11 bg-accent text-white px-4 rounded-full text-xs font-semibold hover:bg-accent-dark transition-colors cursor-pointer"
                  >
                    <Mail className="w-3.5 h-3.5" aria-hidden /> Reply by email
                  </a>
                  {selected.status === "archived" ? (
                    <button
                      disabled={busyId === selected.id}
                      onClick={() => setStatus(selected, "read")}
                      className="inline-flex items-center gap-1.5 min-h-11 border border-border text-text-secondary px-4 rounded-full text-xs font-semibold hover:border-primary hover:text-primary transition-colors cursor-pointer disabled:opacity-60"
                    >
                      <RotateCcw className="w-3.5 h-3.5" aria-hidden /> Unarchive
                    </button>
                  ) : (
                    <button
                      disabled={busyId === selected.id}
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 min-h-11 bg-red-50 text-red-700 px-4 rounded-full text-xs font-semibold hover:bg-red-100 transition-colors cursor-pointer disabled:opacity-60"
                  >
              ...
              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                ...
                >
                  ...
                  <ExternalLink className="w-3 h-3 opacity-50" aria-hidden />
                </a>
                {selected.phone && (
                  <a
      ...
      <ConfirmDialog
        ...
        message={confirmDelete ? `The inquiry from ${confirmDelete.name} will be permanently deleted. This cannot be undone.` : ""}
